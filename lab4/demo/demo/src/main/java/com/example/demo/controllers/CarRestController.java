...
@RestController
@RequestMapping("cars") // localhost:8080/car
public class CarRestController {
    ...
    @PostMapping()
    public ResponseEntity<Car> saveCar(@RequestBody Car person){
        service.saveCar(person);
        return ResponseEntity.noContent().build();
    }

    @PutMapping("{id}")
    public ResponseEntity<Car> updateCar(@PathVariable("id") int id, @RequestBody Car person){

        Car result = service.Update(id, person);
